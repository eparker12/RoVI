...
```{r, message=FALSE}
##############################
## Identical script can be run for EC and pathway modules
##############################
...
# read in metadata
metadata = read.csv("inputs_EC/metadata.csv", header=T)
...
```{r}
system("mkdir outputs_EC")

# import phyloseq table and taxonomy, then merge
otu_biom = read.csv("inputs_EC/biom.csv", header=T, row.names = 1)
...
taxa = read.csv("inputs_EC/taxonomy.csv", header=T, row.names = 1) 
...
* mean % reads unintegrated: `r round(mean(h$ungrouped)*100,1)`
* mean % reads mapped: `r round(mean(1-(h$ungrouped+h$unmapped))*100,1)`
...
system("mkdir outputs_EC")
save(ps_full, file="outputs_EC/final_ps_full.RData")
```
...
Number of functional pathways included: `r ncol(h_prev)`    
