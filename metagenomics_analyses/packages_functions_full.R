...
corr_by_country_alpha = function(df, outcome_measure, country_exposure_subset, sample_subset, return_val) {
  s1 = subset(df, sample_type==sample_subset & !is.na(outcome_measure))
  res = c(corr_function(s1, outcome_measure, "Shannon_ps1", country_exposure_subset, return_option=return_val),
          corr_function(s1, outcome_measure, "Observed_ps1", country_exposure_subset, return_option=return_val))
}
...
corr_function = function(df = s, outcome_measure, covariate, country_exposure_subset, return_option=c("corr", "p", "n")) {
  ...
  cor = cor.test(t[,outcome_measure], t[,covariate], method = "spearman")
  ...
  if (return_option=="n") { return(nrow(t)) }
}
...
feature_abundance_function = function(input_rps, input_country="India", input_colour=India_col, yannotation=0.1) {
  
  sample_list = rownames(sample_data(input_rps)[sample_data(input_rps)$country==input_country])
  ...
  feature_abund = data.frame(round(rowMeans(otu_table(rps_t_no0), na.rm=T),6), round(matrixStats::rowSds(otu_table(rps_t_no0), na.rm=T),6), rowSums(otu_table(rps_t)>0))
  names(feature_abund) = c("mean_abundance", "SD_abundance", "present")
  feature_abund$prevalence = round(feature_abund$present/ncol(otu_table(rps_t)),3)
  feature_abund$feature = data.frame(as(tax_table(rps_t),"matrix"))$feature
  
  # order by decreasing abundance, add rank
  feature_abund = feature_abund[order(-feature_abund$mean_abundance),]
  feature_abund$rank = 1:nrow(feature_abund)
  N_over_50_prev = sum(feature_abund$prevalence>=0.5)
  N_total = nrow(feature_abund)
  
  # Calculate cumulative abundance distribution
  feature_abund$cumulative = NA
  feature_abund$cumulative[1] = feature_abund$mean_abundance[1]
  for (i in 2:nrow(feature_abund)) { feature_abund$cumulative[i] = feature_abund$cumulative[i-1] + feature_abund$mean_abundance[i] }
  feature_abund$label = feature_abund$feature
  feature_abund$label[feature_abund$prevalence<0.9] = NA
  ggplot(feature_abund, aes(x=prevalence, y=mean_abundance, label=label)) +
   ...
   # geom_text(size = 3, colour=input_colour, check_overlap = TRUE, nudge_y=0.01) +
    ...
    annotate("text", x = 0.55, y = 0.045, label = paste0(N_over_50_prev, "/", N_total, " features \nwith prevalence ≥50%"), colour=input_colour,  fontface = 'italic') +
...
summary_abund_plot= function(country1,country_short1,country2,country_short2,colour1, colour2, level, threshold=0.05, yshift=2, ymax=85) {
  ...
  BS3 = read.csv(paste0("output_module3/RF_output_importance/crossval_gini_",country1,"_",country2,"_BS3_",level,".csv"), row.names=1)
  BS5 = read.csv(paste0("output_module3/RF_output_importance/crossval_gini_",country1,"_",country2,"_BS5_",level,".csv"), row.names=1)
  BS1$enriched = country1
  BS2$enriched = country1
  BS3$enriched = country1
  BS5$enriched = country1
  BS1$enriched[BS1$mean_rabund_country1<BS1$mean_rabund_country2] = country2
  BS2$enriched[BS2$mean_rabund_country1<BS2$mean_rabund_country2] = country2
  BS3$enriched[BS3$mean_rabund_country1<BS3$mean_rabund_country2] = country2
  BS5$enriched[BS5$mean_rabund_country1<BS5$mean_rabund_country2] = country2
...
# function to plot taxonomic enrichment between countries (based on presence/absence)
summary_abund_plot_BM = function(country1,country_short1,country2,country_short2,colour1, colour2, level, threshold=0.05, yshift=2, ymax=85) {
  BM1 = read.csv(paste0("output_module3_BM_strict/RF_output_importance/crossval_gini_",country1,"_",country2,"_BM1_",level,".csv"), row.names=1)
  BM2 = read.csv(paste0("output_module3_BM_strict/RF_output_importance/crossval_gini_",country1,"_",country2,"_BM2_",level,".csv"), row.names=1)
  BM3 = read.csv(paste0("output_module3_BM_strict/RF_output_importance/crossval_gini_",country1,"_",country2,"_BM3_",level,".csv"), row.names=1)
  BM1$enriched = country1
  BM2$enriched = country1
  BM3$enriched = country1
  BM1$enriched[BM1$mean_rabund_country1<BM1$mean_rabund_country2] = country2
  BM2$enriched[BM2$mean_rabund_country1<BM2$mean_rabund_country2] = country2
  BM3$enriched[BM3$mean_rabund_country1<BM3$mean_rabund_country2] = country2

  # create dataframe summarising taxonomic enrichment results
  aldex_summary = data.frame(
    count = c(sum(BM1$enriched==country1 & (BM1$wi.eBH<threshold | BM1$fisher_padj<threshold), na.rm=TRUE),
              sum(BM2$enriched==country1 & (BM2$wi.eBH<threshold | BM2$fisher_padj<threshold), na.rm=TRUE),
              sum(BM3$enriched==country1 & (BM3$wi.eBH<threshold | BM3$fisher_padj<threshold), na.rm=TRUE),
              sum(BM1$enriched==country2 & (BM1$wi.eBH<threshold | BM1$fisher_padj<threshold), na.rm=TRUE),
              sum(BM2$enriched==country2 & (BM2$wi.eBH<threshold | BM2$fisher_padj<threshold), na.rm=TRUE),
              sum(BM3$enriched==country2 & (BM3$wi.eBH<threshold | BM3$fisher_padj<threshold), na.rm=TRUE),
              sum(BM1$wi.eBH>threshold & BM1$fisher_padj>threshold, na.rm=TRUE),
              sum(BM2$wi.eBH>threshold & BM2$fisher_padj>threshold, na.rm=TRUE),
              sum(BM3$wi.eBH>threshold & BM3$fisher_padj>threshold, na.rm=TRUE)),
    enriched = c(rep(paste0("enriched ",country_short1),3), rep(paste0("enriched ",country_short2),3), rep("ns",3)),
    week = factor(rep(c("1", "7", "11"),3), levels = c("1", "7", "11")),
    total = c(nrow(BM1), nrow(BM2), nrow(BM3))
  )
...
alpha_longitudinal = function(t1, alpha_measure, alpha_title, outcome, outcome_title, exposure=FALSE) {
  ...
  # create plot
  ggplot(t1, aes(x = factor(country), y = alpha_measure, colour=factor(outcome))) +
    ...
    ylab(alpha_title) + xlab("sample subset") +
...
collated_beta_function = function(outcome, unweighted_logical=TRUE) {
  collated_beta = data.frame(t(data.frame(
    IND_BS3 = beta_outcome(rps1, "India", "BS3", outcome, unweighted=unweighted_logical),
    MLW_BS3 = beta_outcome(rps1, "Malawi", "BS3", outcome, unweighted=unweighted_logical),
    INDexp_BS3 = beta_outcome(rps1, "India (exposed)", "BS3", outcome, unweighted=unweighted_logical),
    INDunexp_BS3 = beta_outcome(rps1, "India (unexposed)", "BS3", outcome, unweighted=unweighted_logical)
  )))
  names(collated_beta) = c("R2", "p", "n")
  collated_beta$country = c("IND","MLW","IND (neo+)","IND (neo-)")
  collated_beta$country =  factor(collated_beta$country, levels =  c("IND", "MLW", "IND (neo+)", "IND (neo-)"))
  collated_beta$full = c(1,1,0,0)
  collated_beta
}
...
beta_plot_ORV = function(ps_input, subset, plot_title, country_subset, method=c("Bray-Curtis (weighted)", "Bray-Curtis (unweighted)"), outcome=c("seroconv", "dose1_shedding")) {
  ...
  t$outcome = t[,outcome]
  if (country_subset=="India (exposed)" | country_subset=="India (unexposed)") {
    samplelist = subset(t, country_exposure==country_subset & sample_type==subset)$sample_ID
  } else {
    samplelist = subset(t, country==country_subset & sample_type==subset)$sample_ID
  }
  ps_input = prune_samples(samplelist, ps_input)
  beta = data.frame(sample_data(ps_input))
...
# function to plot taxonomic enrichment between countries (based on presence/absence) - BM
summary_abund_plot_ORV_BM = function(outcome,country,country_short,level, threshold=0.05, output_dir="output_module4_BM_strict") {
  BM1 = read.csv(paste0(output_dir,"/RF_output_importance/crossval_gini_",outcome,"_",country,"_BM1_",level,".csv"), row.names=1)
  BM2 = read.csv(paste0(output_dir,"/RF_output_importance/crossval_gini_",outcome,"_",country,"_BM2_",level,".csv"), row.names=1)
  BM3 = read.csv(paste0(output_dir,"/RF_output_importance/crossval_gini_",outcome,"_",country,"_BM3_",level,".csv"), row.names=1)

  # create dataframe summarising taxonomic enrichment results
  aldex_summary = data.frame(
    count = c(sum(BM1$effect>0 & (BM1$wi.eBH<threshold | BM1$fisher_padj<threshold), na.rm=TRUE),
              sum(BM2$effect>0 & (BM2$wi.eBH<threshold | BM2$fisher_padj<threshold), na.rm=TRUE),
              sum(BM3$effect>0 & (BM3$wi.eBH<threshold | BM3$fisher_padj<threshold), na.rm=TRUE),
              sum(BM1$effect<0 & (BM1$wi.eBH<threshold | BM1$fisher_padj<threshold), na.rm=TRUE),
              sum(BM2$effect<0 & (BM2$wi.eBH<threshold | BM2$fisher_padj<threshold), na.rm=TRUE),
              sum(BM3$effect<0 & (BM3$wi.eBH<threshold | BM3$fisher_padj<threshold), na.rm=TRUE),
              sum(BM1$wi.eBH>threshold & BM1$fisher_padj>threshold, na.rm=TRUE),
              sum(BM2$wi.eBH>threshold & BM2$fisher_padj>threshold, na.rm=TRUE),
              sum(BM3$wi.eBH>threshold & BM3$fisher_padj>threshold, na.rm=TRUE)),
    enriched = c(rep(paste0("enriched R"),3), rep(paste0("enriched NR"),3), rep("ns",3)),
    week = factor(rep(c("1", "7", "11"),3), levels = c("1", "7", "11")),
    total = c(nrow(BM1), nrow(BM2), nrow(BM3)),
    country = country_short
  )
...
summary_abund_plot_IgA_BM = function(country,country_short,level, threshold=0.05, output_dir="output_module4_BM_strict") {
  BM1 = read.csv(paste0(output_dir,"/RF_output_importance/crossval_regression_IgA_",country,"_BM1_",level,".csv"), row.names=1)
  BM2 = read.csv(paste0(output_dir,"/RF_output_importance/crossval_regression_IgA_",country,"_BM2_",level,".csv"), row.names=1)
  BM3 = read.csv(paste0(output_dir,"/RF_output_importance/crossval_regression_IgA_",country,"_BM3_",level,".csv"), row.names=1)

  # create dataframe summarising taxonomic enrichment results
  aldex_summary = data.frame(
    count = c(sum(BM1$spearman.erho>0 & BM1$spearman.eBH<threshold, na.rm=TRUE),
              sum(BM2$spearman.erho>0 & BM2$spearman.eBH<threshold, na.rm=TRUE),
              sum(BM3$spearman.erho>0 & BM3$spearman.eBH<threshold, na.rm=TRUE),
              sum(BM1$spearman.erho<0 & BM1$spearman.eBH<threshold, na.rm=TRUE),
              sum(BM2$spearman.erho<0 & BM2$spearman.eBH<threshold, na.rm=TRUE),
              sum(BM3$spearman.erho<0 & BM3$spearman.eBH<threshold, na.rm=TRUE),
              sum(BM1$spearman.eBH>threshold, na.rm=TRUE),
              sum(BM2$spearman.eBH>threshold, na.rm=TRUE),
              sum(BM3$spearman.eBH>threshold, na.rm=TRUE)),
    enriched = c(rep(paste0("enriched R"),3), rep(paste0("enriched NR"),3), rep("ns",3)),
    week = factor(rep(c("1", "7", "11"),3), levels = c("1", "7", "11")),
    total = c(nrow(BM1), nrow(BM2), nrow(BM3)),
    country = country_short
...
zinb_ORV_function = function(outcome=c("age", "seroconv", "IgA", "dose1_shedding", "mode_delivery"), input_df, include_nb=TRUE) {
  if (outcome=="age") {
    # ZINB
    f <- mms(y = features, fixed = ~ week + offset(log(count_ps7)), random = ~ 1 | family_ID, data = input_df,
             min.p = 0, method = "zinb", verbose=TRUE)
    res = data.frame(get.fixed(f, part="dist", vr.name="week", sort=F))
    res$method = "zinb"
    
    # NB (if prevalence >95%)
    f_nb <- mms(y = features, fixed = ~ week + offset(log(count_ps7)), random = ~ 1 | family_ID, data = input_df,
                min.p = 0.95, method = "nb", verbose=TRUE)
    res_nb = data.frame(get.fixed(f_nb, part="dist", vr.name="week", sort=F))
    res_nb$method = "nb"
  ...
  if (outcome=="seroconv") {
    # ZINB
    f <- mms(y = features, fixed = ~ seroconv + week + offset(log(count_ps7)), data = input_df,
             random = ~ 1 | family_ID,
             min.p = 0, method = "zinb", verbose=TRUE)
    res <- data.frame(get.fixed(f, part="dist", vr.name="seroconv", sort=F))
    res$method = "zinb"
    
    # NB (if prevalence >95%)
    f_nb = mms(y = features, fixed = ~ seroconv + week + offset(log(count_ps7)), random = ~ 1 | family_ID, data = input_df,
               min.p = 0.95, method = "nb", verbose=TRUE)
    res_nb = data.frame(get.fixed(f_nb, part="dist", vr.name="seroconv", sort=F))
    res_nb$method = "nb"
    for (i in 1:nrow(res_nb)){
      if (rownames(res_nb)[i] %in% rownames(res)) { res[rownames(res_nb)[i],] = res_nb[i,] } else { res = rbind(res, res_nb[i,]) }
    }
  ...
  if (outcome=="dose1_shedding") {
    # ZINB
    f <- mms(y = features, fixed = ~ dose1_shedding + week + offset(log(count_ps7)), data = input_df,
             random = ~ 1 | family_ID,
             min.p = 0, method = "zinb", verbose=TRUE)
    res <- data.frame(get.fixed(f, part="dist", vr.name="dose1_shedding", sort=F))
    res$method = "zinb"
    
    # NB (if prevalence >95%)
    f_nb = mms(y = features, fixed = ~ dose1_shedding + week + offset(log(count_ps7)), data = input_df,
               random = ~ 1 | family_ID,
               min.p = 0.95, method = "nb", verbose=TRUE)
    res_nb = data.frame(get.fixed(f_nb, part="dist", vr.name="dose1_shedding", sort=F))
    res_nb$method = "nb"
    for (i in 1:nrow(res_nb)){
      if (rownames(res_nb)[i] %in% rownames(res)) { res[rownames(res_nb)[i],] = res_nb[i,] } else { res = rbind(res, res_nb[i,]) }
    }
  ...
  if (outcome=="IgA") {
    # ZINB
    f <- mms(y = features, fixed = ~ IgA + week + offset(log(count_ps7)), data = input_df,
             random = ~ 1 | family_ID,
             min.p = 0, method = "zinb", verbose=TRUE)
    res <- data.frame(get.fixed(f, part="dist", vr.name="IgA", sort=F))
    res$method = "zinb"
    
    # NB (if prevalence >95%)
    f_nb = mms(y = features, fixed = ~ IgA + week + offset(log(count_ps7)), random = ~ 1 | family_ID, data = input_df,
               min.p = 0.95, method = "nb", verbose=TRUE)
    res_nb = data.frame(get.fixed(f_nb, part="dist", vr.name="IgA", sort=F))
    res_nb$method = "nb"
    for (i in 1:nrow(res_nb)){
      if (rownames(res_nb)[i] %in% rownames(res)) { res[rownames(res_nb)[i],] = res_nb[i,] } else { res = rbind(res, res_nb[i,]) }
    }
  ...
  if (outcome=="mode_delivery") {
    # ZINB
    f <- mms(y = features, fixed = ~ mode_delivery + week + offset(log(count_ps7)), data = input_df,
             random = ~ 1 | family_ID,
             min.p = 0, method = "zinb", verbose=TRUE)
    res <- data.frame(get.fixed(f, part="dist", vr.name="mode_deliveryvaginal", sort=F))
    res$method = "zinb"
    
    if (include_nb==TRUE) {
    # NB (if prevalence >95%)
    f_nb = mms(y = features, fixed = ~ mode_delivery + week + offset(log(count_ps7)), random = ~ 1 | family_ID, data = input_df,
               min.p = 0.95, method = "nb", verbose=TRUE)
    res_nb = data.frame(get.fixed(f_nb, part="dist", vr.name="mode_deliveryvaginal", sort=F))
    res_nb$method = "nb"
    for (i in 1:nrow(res_nb)){
      if (rownames(res_nb)[i] %in% rownames(res)) { res[rownames(res_nb)[i],] = res_nb[i,] } else { res = rbind(res, res_nb[i,]) }
    }
...
modules_importance_plot = function(path, binary=TRUE) {
  ...
  importance$variable = gsub("water treat wami", "treated water", importance$variable)
...
summarise_regression = function(variable, input_data, log=FALSE) {
  ...
  if (nrow(r)>=10 & nrow(nr)>=10) {
    ...
    if(class(input_sub$variable)=="integer" | class(input_sub$variable)=="numeric") {
      ...
      if (log==TRUE & variable!="Observed" & variable!="Observed_6w" & variable!="Observed_10w" & variable!="houshold_income") {
...
baseline_summary_alpha = function(input_data) {
  ...
  if (any(input_data$pre_exposed == "yes", na.rm=T) & any(input_data$pre_exposed == "no", na.rm=T)) {
