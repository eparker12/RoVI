...
Sys.setenv("R_MAX_VSIZE" = 8e9) #check assignment using Sys.getenv("R_MAX_VSIZE")
...
if (run_full) {
  ...
  taxa_bac = subset(taxa_length, Kingdom=="Bacteria" & Phylum!="Cyanobacteria" & Family!="Mitochondria")
  ps3_l_t = prune_taxa(rownames(taxa_bac), ps2_l) %>% prune_samples(sample_sums(.) > 0, .)
  sample_data(ps3_l_t)$count_ps3 = sample_sums(ps3_l_t)
  
  # save phyloseq object
  save(ps3_l_t, file="phyloseq_files/ps3_l_t.RData")
  rm(ps2_l, taxa_length, taxa_bac)
} else {
...
gg_add = function(plot) {
  plot + theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(size=10)) + 
...
p1 = gg_add(ggplot(data=subset(t, sample_type=="NTCs"), aes(x=sample_type, y=count_ps4)) + geom_jitter(alpha = 0.8, width=0.2, color="darkgrey") + 
  ylab("reads") + labs(colour="sample type") + scale_colour_brewer(palette="Set2") + 
  ggtitle("NTCs"))
...
p2 = gg_add(ggplot(data=subset(t, sample_type=="WCS" & type=="pool"), aes(x=country, y=count_ps4, color=country)) + geom_jitter(alpha = 0.8, width=0.2) + 
  scale_color_manual(values = c("India" = India_col, "Malawi" = Malawi_col, "UK" = UK_col)) + ylab("") + 
...
gg_add = function(plot) { 
  plot + theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(size=10)) + 
    geom_hline(yintercept = 10000, linetype="dotted") + ylim(-1,500000) + xlab("") 
  }
...
p1 = gg_add(ggplot(data=subset(t1, sample_type=="NTCs"), aes(x=sample_type, y=count_ps4)) + geom_jitter(alpha = 0.8, width=0.2, color="darkgrey") + 
  ylab("reads") + labs(colour="sample type") + scale_colour_brewer(palette="Set2") + ggtitle("NTCs"))
...
p1 = ggplot(beta, aes(bray1, bray2, colour=type)) + geom_point(size = 3, alpha = 0.8) + 
  ...
  ylim(-0.2, 0.2) + xlim(-0.75, 0.5) +
  scale_colour_manual(values = c(pool="#01665e", single="#80cdc1")) +
  ggtitle("Bray-Curtis distances (weighted)") + theme(plot.title = element_text(size=10))
...
ggplot(data=subset(t, sample_group!="ctrl" & site=="Liverpool"), aes(x=country, y=nanodrop_ngul, color=country)) + geom_jitter(alpha = 0.2, width=0.2) + 
  geom_boxplot(alpha = 0.8) + facet_grid(.~sample_group_fig) + 
...
# pick out controls
ps8_controls = subset_samples(ps8_genus, site=="Liverpool" & 
...
r2 <- format(round(summary(lm(Shannon ~ sample_type, data = alpha))$r.squared,3),nsmall=3)
p1 = ggplot(alpha, aes(factor(sample_type), y=Shannon, colour=factor(sample_type))) + geom_jitter(size = 1, alpha = 0.8, width=0.2) +
  geom_boxplot(alpha = 0.5) + ylab("Shannon (genus)") + xlab("") + theme(legend.position = "none", strip.background = element_blank()) + ylim(0,3) +
  scale_color_manual(values=c(BSctrl = "#5e4fa2", MCQia="#3288bd", MCctrl = "#66c2a5", MSctrl = "#fee08b")) +
  ggtitle(bquote(R^2 == .(r2))) + theme(plot.title = element_text(size=10))

# generate richness plot with R-sq 
r2 <- round(summary(lm(Observed ~ sample_type, data = alpha))$r.squared,3)
p2 = ggplot(alpha, aes(factor(sample_type), y=Observed, colour=factor(sample_type))) + geom_jitter(size = 1, alpha = 0.8, width=0.2) +
...
bray_dist = phyloseq::distance(ps8_sub, method = "bray")
bray_ord = ordinate(ps8_sub, method = "PCoA", distance = bray_dist)
beta$bray1 = bray_ord$vectors[,1]
beta$bray2 = bray_ord$vectors[,2]
r2 = round(adonis(bray_dist ~ sample_type, data = beta, permutations = 999)$aov.tab$R2[1],3)
p3 = ggplot(beta, aes(bray1, bray2, colour=sample_type)) + geom_point(size = 2, alpha = 0.5) + 
  scale_color_manual(values=c(BSctrl = "#5e4fa2", MCQia="#3288bd", MCctrl = "#66c2a5", MSctrl = "#fee08b")) +
...
bray_dist = phyloseq::distance(rps8_sub, method = "bray", binary=TRUE)
bray_ord = ordinate(rps8_sub, method = "PCoA", distance = bray_dist)
beta$bray1 = bray_ord$vectors[,1]
beta$bray2 = bray_ord$vectors[,2]
r2 = format(round(adonis(bray_dist ~ sample_type, data = beta, permutations = 999)$aov.tab$R2[1],3),nsmall=3)
p4 = ggplot(beta, aes(bray1, bray2, colour=sample_type)) + geom_point(size = 2, alpha = 0.5) + 
  scale_color_manual(values=c(BSctrl = "#5e4fa2", MCQia="#3288bd", MCctrl = "#66c2a5", MSctrl = "#fee08b")) +
  ggtitle(bquote("unweighted Bray-Curtis,"~R^2==.(r2))) + theme(legend.title = element_blank(), plot.title = element_text(size=10)) + 
...
for (i in 1:nrow(collated)) {
  ...
  ps8_sub = prune_samples(sample_list, rps8_genus) %>% prune_taxa(taxa_sums(.) > 0, .)
  rps8_sub = transform_sample_counts(ps8_sub, function(x) {x/sum(x)})
  ...
  # weighted adonis
  bray_dist_w = phyloseq::distance(ps8_sub, method = "bray")
...
collated$sample_type = rep(c("week 1", "week 4", "week 6/8", "week 10/12", "mother"),3)
collated$sample_type = factor(collated$sample_type, levels = c("week 1", "week 4", "week 6/8", "week 10/12", "mother"))

# generate weighted Bray-Curtis plot with R-sq
r2 = round(mean(collated$R2_w),3)
p1 = ggplot(collated, aes(y = R2_w, x=sample_type, fill = country)) + geom_bar(stat = "identity") + facet_grid(.~country) + 
  scale_x_discrete(limits = rev(levels(collated$sample_type))) + xlab("") + ylab(bquote(R^2~", weighted distances")) + ylim(0,0.2) + 
  geom_hline(yintercept = mean(collated$R2_w), linetype="dotted") + coord_flip() + 
  scale_fill_manual(values = c("India" = India_col, "Malawi" = Malawi_col, "UK" = UK_col)) + theme(legend.position = "none") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle(bquote("mean"~R^2~"for run ="~.(r2))) + theme(plot.title = element_text(size=10)) + 
...
p2 = ggplot(collated, aes(y = R2_u, x= sample_type , fill = country)) + geom_bar(stat = "identity") + facet_grid(.~country) + 
  scale_x_discrete(limits = rev(levels(collated$sample_type))) + xlab("") + ylab(bquote(R^2~", unweighted distances")) + ylim(0,0.2) + 
  geom_hline(yintercept = mean(collated$R2_w), linetype="dotted") + coord_flip() + 
  scale_fill_manual(values = c("India" = India_col, "Malawi" = Malawi_col, "UK" = UK_col)) + theme(legend.position = "none") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  ggtitle(bquote("mean"~R^2~"for run ="~.(r2))) + theme(plot.title = element_text(size=10)) + 
...
# create multi-panel plot
grid.arrange(p1,p2)
rm(p1, p2, adon, ps8_sub, rps8_sub, beta)
```
...
p3 = ggplot(beta, aes(bray1, bray2, colour=sample_type)) + geom_point(size = 2, alpha = 0.7) + 
  ...
  ggtitle(bquote("weighted Bray-Curtis,"~R^2==.(r2))) + theme(legend.position = "none", plot.title = element_text(size=10)) + xlab(paste0("PC1 (", format(round(bray_ord$values$Relative_eig[1]*100,1),nsmall=1), "%)")) + ylab(paste0("PC2 (", round(bray_ord$values$Relative_eig[2]*100,1), "%)"))
...
p4 = ggplot(beta, aes(bray1, bray2, colour=sample_type)) + geom_point(size = 2, alpha = 0.7) + 
  scale_color_manual(values=c(BMctrl = "#5e4fa2", MCQiaBM="#e6550d", MCctrlBM = "#66c2a5")) +
  ggtitle(bquote("unweighted Bray-Curtis,"~R^2==.(r2))) + theme(legend.title = element_blank(), plot.title = element_text(size=10)) + xlab(paste0("PC1 (", round(bray_ord$values$Relative_eig[1]*100,1), "%)")) + ylab(paste0("PC2 (", format(round(bray_ord$values$Relative_eig[2]*100,1),nsmall=1), "%)"))
...
# recode sample type for plot
collated$sample_type = rep(c("week 1", "week 7/9", "week 11/13"),3)
collated$sample_type = factor(collated$sample_type, levels = c("week 1", "week 7/9", "week 11/13"))
...
m1 = data.frame(sample_data(ps5_genus))
m1$filtered_count_ps5 = sample_sums(ps5_genus)
load("phyloseq_files/ps8_genus.RData")
m2 = data.frame(sample_data(ps8_genus))
...
# pick out subsets
m_val = data.frame(sample_data(ps5_genus_val_rarefied))
m_val_imp = data.frame(subset(m_val, site=="London"))
m_val_cgr = data.frame(subset(m_val, site=="Liverpool"))

# calculate alpha diversity
alpha_df = estimate_richness(ps5_genus_val_rarefied, measures = c("Shannon"))
alpha_df$Observed = colSums(otu_table(rps5_genus_val_rarefied)>=0.001)
...
# recode sample type
imp_cgr$sample_type[imp_cgr$sample_type=="BS1"] = "week 1"
imp_cgr$sample_type[imp_cgr$sample_type=="BS5"] = "week 10/12"
imp_cgr$sample_type[imp_cgr$sample_type=="MS1"] = "mother"
imp_cgr$sample_type = factor(imp_cgr$sample_type, levels = c("week 1", "week 10/12", "mother"))
...
p1 = ggplot(imp_cgr, aes(Shannon.x, Shannon.y, colour = sample_type)) + geom_abline(color = "black", linetype = "dotted") + 
  geom_point(size = 1.5, alpha = 0.7) + xlab("Shannon (genus), Liverpool") +  ylab("Shannon (genus), London") + 
  theme_bw() + facet_grid(country~ sample_type) +
  scale_colour_manual(values = c("week 1" = "#fe9929", "week 10/12" = "#cc4c02", "mother" = "#662506", "breastmilk" = "#2c7fb8")) + theme(legend.position = "none") +
  ggtitle(bquote("overall"~R^2==~.(r2))) + theme(legend.position = "none", plot.title = element_text(size=10)) + 
  theme(strip.background = element_blank()) 

# generate richness plot with R-sq
r2 <- round(summary(lm(Observed.y ~ Observed.x, data = imp_cgr))$r.squared,3)
p2 = ggplot(imp_cgr, aes(Observed.x, Observed.y, colour = sample_type)) + geom_abline(color = "black", linetype = "dotted") + 
  geom_point(size = 1.5, alpha = 0.7) + xlab("richness (genus), Liverpool") +  ylab("richness (genus), London") +
  theme_bw() + facet_grid(country~ sample_type) +
  scale_colour_manual(values = c("week 1" = "#fe9929", "week 10/12" = "#cc4c02", "mother" = "#662506", "breastmilk" = "#2c7fb8")) + theme(legend.position = "none") +
  ggtitle(bquote("overall"~R^2==~.(r2))) + theme(legend.position = "none", plot.title = element_text(size=10)) + 
  theme(strip.background = element_blank()) 
plot_grid(p1,p2, align="v", ncol=1)
rm(m_val, m_val_imp, m_val_cgr, alpha_df, p1, p2, ps5_genus_val, ps5_genus_val_rarefied)
```

#### Beta diversity plots
```{r, fig.width=6, fig.height=12}
# generate weighted Bray-Curtis plot for paired samples with R-sq
beta = data.frame(sample_data(rps5_genus_val_rarefied))
bray_dist = phyloseq::distance(rps5_genus_val_rarefied, method = "bray")
bray_ord = ordinate(rps5_genus_val_rarefied, method = "PCoA", distance = bray_dist)
beta$bray1 = bray_ord$vectors[,1]
beta$bray2 = bray_ord$vectors[,2]
r2 = round(adonis(bray_dist ~ sample_code, data = beta, permutations = 999)$aov.tab$R2[1],3)

# recode sample type
beta$sample_type[beta$sample_type=="BS1"] = "week 1"
beta$sample_type[beta$sample_type=="BS5"] = "week 10/12"
beta$sample_type[beta$sample_type=="MS1"] = "mother"
beta$sample_type = factor(beta$sample_type, levels = c("week 1", "week 10/12", "mother"))

p1 = ggplot(beta, aes(bray1, bray2, colour=sample_type)) + geom_point(size = 2, alpha = 0.3) + 
  scale_colour_manual(values = c("week 1" = "#fe9929", "week 10/12" = "#cc4c02", "mother" = "#662506", "breastmilk" = "#2c7fb8")) + 
  geom_line(aes(group = sample_code)) + facet_grid(country~sample_type) +
  xlab(paste0("PC1 (", round(bray_ord$values$Relative_eig[1]*100,1), "%)")) + ylab(paste0("PC2 (", round(bray_ord$values$Relative_eig[2]*100,1), "%)")) +
  ggtitle(bquote("weighted Bray-Curtis,"~R^2~"for sample code ="~.(r2))) + theme(legend.position = "none", plot.title = element_text(size=10)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  theme(strip.background = element_blank()) 

# generate weighted Bray-Curtis plot for paired samples with R-sq
otu_table(rps5_genus_val_rarefied)[otu_table(rps5_genus_val_rarefied)<0.001] = 0
beta = data.frame(sample_data(rps5_genus_val_rarefied))
bray_dist = phyloseq::distance(rps5_genus_val_rarefied, method = "bray", binary=TRUE)
bray_ord = ordinate(rps5_genus_val_rarefied, method = "PCoA", distance = bray_dist)
beta$bray1 = bray_ord$vectors[,1]
beta$bray2 = bray_ord$vectors[,2]
r2 = round(adonis(bray_dist ~ sample_code, data = beta, permutations = 999)$aov.tab$R2[1],3)
...
p2 = ggplot(beta, aes(bray1, bray2, colour=sample_type)) + geom_point(size = 2, alpha = 0.3) + 
  scale_colour_manual(values = c("week 1" = "#fe9929", "week 10/12" = "#cc4c02", "mother" = "#662506", "breastmilk" = "#2c7fb8")) + 
  geom_line(aes(group = sample_code)) + facet_grid(country~sample_type) +
  xlab(paste0("PC1 (", round(bray_ord$values$Relative_eig[1]*100,1), "%)")) + ylab(paste0("PC2 (", round(bray_ord$values$Relative_eig[2]*100,1), "%)")) +
  ggtitle(bquote("unweighted Bray-Curtis,"~R^2~"for sample code ="~.(r2)))  + theme(legend.position = "none", plot.title = element_text(size=10)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  theme(strip.background = element_blank()) 

# create multi-panel figure
plot_grid(p1,p2, align="h", ncol=1)
rm(beta, bray_ord, p1, p2)
```
...
p2 = ggplot(imp_cgr, aes(Observed.x, Observed.y, colour = sample_type)) + geom_abline(color = "black", linetype = "dotted") + 
  ...
  theme(strip.background = element_blank()) 
plot_grid(p1,p2, align="v", ncol=1)
rm(m_val, m_val_imp, m_val_cgr, alpha_df, p1, p2, ps5_genus, ps5_genus_val, ps5_genus_val_rarefied)
```
...
if (all(rownames(h_prev)==sample_names(rps7c))) { 
  annotation_col = data.frame(week = as.factor(paste0(sample_data(rps7c)$sample_type)), 
...
# recode sample type
annotation_col$week = revalue(annotation_col$week, c("BS1"="1", "BS2"="4", "BS3"="6/8", "BS5"="10/12", "MS1"="mother"))

# set annotation colours
sample_cols = c("#fee391", "#fec44f", "#fe9929", "#d95f0e", "#993404")
names(sample_cols) = levels(annotation_col$week)
country_cols = c(India_col, Malawi_col, UK_col)
names(country_cols) = levels(annotation_col$country)
ann_colors = list(week = sample_cols, country = country_cols)
...
pheatmap(t(h_prev), clustering_distance_rows="correlation", clustering_distance_cols="correlation", cluster_cols = T, 
         ...
         annotation_col = annotation_col, annotation_colors = ann_colors, show_rownames=FALSE, show_colnames=FALSE, fontsize=10)
rm(ann_colors, annotation_col, h, h_prev, ps7, ps7_genus, rps7)
```
    
Number of taxa present at ≥0.1% abundance in ≥1% of samples: `r ntaxa(rps7c)`
