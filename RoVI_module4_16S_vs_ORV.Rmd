...
# add log E/B ratio
rps8_family = tax_glom(rps8, "Family", NArm = TRUE)
t = data.frame(sample_data(rps8_family))
df_family = data.frame(t(otu_table(rps8_family)))

if (all(colnames(df_family)==rownames(tax_table(rps8_family)))) { colnames(df_family) = as(tax_table(rps8_family), "matrix")[,"Family"] } else { print("error: rownames do not match")}

if (all(t$sample_ID_full==rownames(df_family))) {
  t$Bacteroidaceae = df_family$Bacteroidaceae
  t$Enterobacteriaceae = df_family$Enterobacteriaceae
}
t$log_Entero_Bac_ratio =  log((t$Enterobacteriaceae+0.001)/(t$Bacteroidaceae+0.001))

if (all(sample_data(rps8)$sample_ID_full == t$sample_ID_full)) {
  sample_data(rps8) = sample_data(rps8_genus) = sample_data(rps7) = sample_data(rps7_genus) = t
  sample_data(ps8) = sample_data(ps8_genus) = sample_data(ps7) = sample_data(ps7_genus) = t
}
```
...
write.csv(t, "final_analysis_set_metadata.csv")
...
```{r, fig.width=10, fig.height=3.5}
# generate plots
sero_fig_A_alt = alpha_longitudinal(t_plot, "Observed", "richness", "seroconv", "seroconversion", exposure=FALSE) + ylim(0,30) + 
  ggtitle("alpha diversity") + theme(plot.title = element_text(size=16, hjust=0.5))
sero_fig_A_alt
```

#### Longitudinal plots - E/B ratio
```{r, fig.width=10, fig.height=3.5}
# generate plots
sero_fig_A_alt = alpha_longitudinal(t_plot, "log_Entero_Bac_ratio", "log(Ent/Bac)", "seroconv", "seroconversion", exposure=FALSE) + 
  ggtitle("alpha diversity") + theme(plot.title = element_text(size=16, hjust=0.5))
sero_fig_A_alt
```
...
##### IND
```{r}
t2 = subset(t1, country=="IND" & sample_type=="BS3")
wilcox.test(log_Entero_Bac_ratio ~ seroconv, data = t2)
```

##### MLW
```{r}
t2 = subset(t1, country=="MLW" & sample_type=="BS3")
wilcox.test(log_Entero_Bac_ratio ~ seroconv, data = t2)
```

##### UK
```{r}
t2 = subset(t1, country=="UK" & sample_type=="BS3")
wilcox.test(log_Entero_Bac_ratio ~ seroconv, data = t2)
```

##### IND neo+
```{r}
t2 = subset(t1, country_exposure=="IND (neo+)" & sample_type=="BS3")
wilcox.test(log_Entero_Bac_ratio ~ seroconv, data = t2)
```
...
```{r, fig.width=8, fig.height=3.5}
# generate plots
dose1_fig_A_alt = alpha_longitudinal(t_plot, "Observed", "richness", "dose1_shedding", "dose 1 shedding", exposure=FALSE) + ylim(0,30)  + 
  ggtitle("alpha diversity") + theme(plot.title = element_text(size=16, hjust=0.5))
dose1_fig_A_alt
```
