...
run_rf = function(input_rps, input_ps, prev_threshold=0.05, folds=5, iterations=iter, n_per_group=50) {
  ...
  if (any(gini.cv.r$fisher_padj<0.05 | gini.cv.r$wi.eBH<0.05)) { 
    ...
    gini.cv.r = gini.cv.r %>% arrange(pmin)  %>% select(-pmin)
    write.csv(gini.cv.r, file=paste0("output_module3/RF_output_importance_reduced/crossval_gini_",country1,"_",country2,"_",subset,"_",level,".csv"))
  }
...
run_rf_binary_outcome = function(input_rps, input_ps, country_arm, sample_sub, prev_threshold=0.05, folds=5, iterations=iter, n_per_group=50) {
  ...
  for (i in 1:nrow(gini.cv)) {
    ...
    gini.cv$nonresponder_prev[i] = round(sum(rf_input_1[,taxon]>0)/nrow(rf_input_1)*100,1)
    gini.cv$nonresponder_mean[i] = round(mean(rf_input_1[,taxon])*100,1)
    gini.cv$nonresponder_sd[i] = round(sd(rf_input_1[,taxon])*100,1)
    
    gini.cv$responder_present[i] = sum(rf_input_2[,taxon]>0)
    gini.cv$responder_n[i] = nrow(rf_input_2)
    gini.cv$responder_prev[i] = round(sum(rf_input_2[,taxon]>0)/nrow(rf_input_2)*100,1)
    gini.cv$responder_mean[i] = round(mean(rf_input_2[,taxon])*100,1)
    gini.cv$responder_sd[i] = round(sd(rf_input_2[,taxon])*100,1)
    gini.cv$fisher_p[i] = fisher.test(matrix(c(sum(rf_input_1[,taxon]>0), sum(rf_input_1[,taxon]==0),
                                               sum(rf_input_2[,taxon]>0), sum(rf_input_2[,taxon]==0)),nrow=2))$p.value
  }
  ...
  # add plot colour scheme
  gini.cv$fisher_col = "ns"
  gini.cv$fisher_col[gini.cv$prev_diff>0 & gini.cv$fisher_padj<0.05] = "nonresponder"
  gini.cv$fisher_col[gini.cv$prev_diff<0 & gini.cv$fisher_padj<0.05] = "responder"
  
  # aldex
  aldex_ps = prune_taxa(taxlist, input_ps)
  aldex_rps = transform_sample_counts(aldex_ps, function(x) {x/sum(x)})
  aldex_rps_nonresponders =  prune_samples(ps1list, aldex_rps)
  aldex_rps_responders =  prune_samples(ps2list, aldex_rps)
  features = data.frame(otu_table(aldex_ps))
  rfeatures = data.frame(otu_table(aldex_rps))
  groups = sample_data(aldex_ps)$outcome
  x.all <- aldex(features, groups, test="t", effect=TRUE, include.sample.summary=FALSE, denom="all", verbose=FALSE)
  
  for (i in 1:nrow(features)) { 
    x.all$mean_rabund_nonresponders[i] = round(mean(otu_table(aldex_rps_nonresponders)[i,]*100),3)
    x.all$mean_rabund_responders[i] =  round(mean(otu_table(aldex_rps_responders)[i,]*100),3)
  }
  if (level == "genus") { x.all$tax_id = data.frame(as(tax_table(aldex_ps), "matrix"))$Genus }
  if (level == "rsv") { x.all$tax_id = data.frame(as(tax_table(aldex_ps), "matrix"))$tax_id }
  gini.cv = merge(gini.cv, x.all, by = "tax_id", all=TRUE)
  ...
  # convert to DESeq2 object
  # ds = phyloseq_to_deseq2(aldex_ps, ~ outcome)
...
collate_rf_binary = function(output_name) {
  ...
  for (s in 1:length(subset_list)) {
    for (c in 1:length(country_list)) {
      ...
      input_ps = prune_samples(samplelist, ps)
      
      sample_data(input_rps)$outcome = as(sample_data(input_rps), "matrix")[,outcome]
      sample_data(input_ps)$outcome = as(sample_data(input_ps), "matrix")[,outcome]
      rf = run_rf_binary_outcome(input_rps, input_ps, country_arm=country_subset, sample_sub=sample_subset, prev_threshold=0.05, folds=5, iterations=20, n_per_group=50)
...
collate_rf_IgA = function(output_name) {
  ...
  for (s in 1:length(subset_list)) {
    for (c in 1:length(country_list)) {
      ...
      taxlist = rownames(tax_table(input_rps))
      ...
      for (i in 1:nrow(IncMSE_output)) {
        taxon = rownames(IncMSE_output)[i]
        IncMSE_output$present[i] = sum(rf_input[,taxon]>0)
        IncMSE_output$n[i] = nrow(rf_input)
        IncMSE_output$prev[i] = round(sum(rf_input[,taxon]>0)/nrow(rf_input)*100,1)
        IncMSE_output$mean[i] = round(mean(rf_input[,taxon])*100,1)
        IncMSE_output$rho[i] = cor.test(log_IgA, rf_input[,taxon], method="spearman")$estimate
        IncMSE_output$spearman_p[i] = cor.test(log_IgA, rf_input[,taxon], method="spearman")$p.value
      }
      
      # calculate fdr-adjusted p values - presence or absence
      IncMSE_output$spearman_p_adj = p.adjust(IncMSE_output$spearman_p, method="BH")
      IncMSE_output$tax_id = rownames(IncMSE_output)
      ...
      features = data.frame(otu_table(aldex_ps))
      rfeatures = data.frame(otu_table(aldex_rps))
      cont.var = log(sample_data(aldex_ps)$BB2_IgA)
      x <- aldex.clr(features)
      corr.test <- aldex.corr(x, cont.var, method="spearman")
      
      for (i in 1:nrow(features)) { corr.test$mean_rabund[i] = round(mean(otu_table(aldex_rps)[i,]*100),3) }
      if (level == "genus") { corr.test$tax_id = data.frame(as(tax_table(aldex_ps), "matrix"))$Genus }
      if (level == "rsv") { corr.test$tax_id = data.frame(as(tax_table(aldex_ps), "matrix"))$taxonomy }
      IncMSE_output = merge(IncMSE_output, corr.test, by = "tax_id", all=TRUE)
...
rf_modules_binary = function(module, module_file, outcome, outcome_file, iterations = iter, n_per_group=50, folds=5) {
  ...
  # add cross-val median + IQR
  gini.cv$crossval_median = median(stat.cv$accuracy)
  gini.cv$crossval_q1 = quantile(stat.cv$accuracy)[2]
  gini.cv$crossval_q3 = quantile(stat.cv$accuracy)[4]
